...
| `POST` | `/api/sprints` | Cadastra um sprint (`Nome`, `HorasPorDia`, `MargemPercentual`, `DataInicio`, `DataFim`, `DiasNaoUteis` opcional) — 400 (nome vazio, datas inválidas, `fim < inicio`, `HorasPorDia` ausente ou `<= 0`, `MargemPercentual` ausente ou fora de `[0, 100)`, `DiasNaoUteis` fora de `[0, dias úteis do período]`) / 409 (nome em uso); nasce aberto |
| `PUT` | `/api/sprints/{chave}` | Edita um sprint (campos `null`/vazios não alteram); 409 se estiver fechado |
| `PUT` | `/api/sprints/{chave}/deducoes` | `{ horasDeduzidas: { <chaveToggl>: horas } }` substitui as horas deduzidas da capacidade por colaborador (férias, folga, atestado; inteiros `>= 0`, zero remove); 400 negativo, 409 se fechado |
| `DELETE` | `/api/sprints/{chave}` | Remove o sprint **e** seus caches (`SprintData_<chave>` e `JiraSprintData_<chave>`); o Planejamento é global e não é afetado |
...
| `GET` | `/api/sprint?chaveSprint=` | Acompanhamento do sprint (capacidade, colaboradores, grid, `tempoPorTag`); 409 sem cache para o período do sprint |
| `POST` | `/api/planejamento/consultas` | `{ quadro: dev\|analise, forcar }` → `{ veioDoCache, atualizadoEm, quadroNome, sprintJiraNome, quantidadeCartoes }`. Reaproveita o cache global do quadro ou busca ao vivo os cartões do sprint ativo dele (ver [Planejamento](#planejamento)); grava `JiraPlanejamentoGlobalDev`/`JiraPlanejamentoGlobalAnalise`. 400 (quadro inválido, sem quadro/conexão/campos configurados), 502 (falha do Jira) |
| `GET` | `/api/planejamento?quadro=dev\|analise` | Planejamento montado a partir do cache do quadro (`nomeQuadro`, `sprintJiraNome`/`Inicio`/`Fim`, `atualizadoEm`, `colunas`, `totaisPorColuna`, `cartoes`, `colaboradores`); sem `quadro` = `dev`; 409 sem cache do quadro configurado — nunca chama o Jira |
| `GET/PUT` | `/api/jira/configuracao` | URL do domínio, e-mail, quadros de DEV e de Análise (`quadroDevId`/`quadroDevNome`, `quadroAnaliseId`/`quadroAnaliseNome`), campos de estimativa (desenvolvimento/revisão/testes), "revisado por", "analisado por" e "time"; token sempre mascarado. `PUT` **substitui a configuração inteira**; `apiToken` omitido/vazio mantém o salvo (400 se nunca houve um) |
...
| `GET` | `/api/jira/colunas?quadro=dev\|analise&forcarAtualizacao=` | Nomes reais das colunas do quadro configurado (`GET /rest/agile/1.0/board/{id}/configuration`), cache `JiraColunasDevCache`/`JiraColunasAnaliseCache` com o `quadroId` (trocar o quadro invalida o cache); 400 sem configuração do Jira, quadro inválido ou sem quadro selecionado |
| `GET` | `/api/jira/epicos` | Épicos (`{ chave, resumo }`) encontrados nos dois caches globais do Planejamento (DEV e Análise), sem chamar o Jira; sem repetição por chave. A cor salva de um épico que sumiu continua em `JiraEpicos` (só não é listada) |
| `GET` | `/api/jira/times` | Times (campo Time) encontrados nos dois caches globais do Planejamento, sem chamar o Jira — o campo não tem listagem confiável na API |
...
| `GET/PUT` | `/api/jira/cores` | Cor (hex) por nome de status, de prioridade, de coluna (por quadro) e de time, e por chave de épico: `{ coresStatus, coresPrioridade, coresColunaDev, coresColunaAnalise, coresTime, coresEpico }` |
| `GET/PUT` | `/api/jira/quadro` | Colunas de cada quadro que não devem aparecer no Planejamento: `{ colunasOcultasDev, colunasOcultasAnalise }` (ver [Planejamento](#planejamento)) |
...
| `GET` | `/api/jira/quadros?forcarAtualizacao=` | Quadros **Scrum e Kanban** reais do Jira (`GET /rest/agile/1.0/board`, paginado por `isLast`; outros tipos ficam de fora), com `tipo`, cache `JiraQuadrosCache`; 400 sem configuração do Jira, 502 se o Jira falhar |

**Erros**: 400 (parâmetros/datas inválidos), 404 (recurso não encontrado), 409 (nome em uso; sem cache correspondente; sprint fechado), 500 (`Results.Problem`, falha de I/O), 502 (falha do Jira, também via `Results.Problem`). `GET /api/relatorio`, `/busca`, `/gant`, `/sprint` e `/planejamento` **exigem cache prévio** — nunca disparam consulta implícita, para manter explícito quando uma chamada externa ao Toggl acontece (rate limit).
...
`/api/sprints` e `/api/sprint/*` são a terceira visualização. Diferente do relatório e do Gant, tem **gestão** (CRUD de sprints) além do acompanhamento. Cada sprint tem `Nome`, `HorasPorDia`, `MargemPercentual`, `DataInicio`, `DataFim`, `DiasNaoUteis` (opcional), `HorasDeduzidas` (por colaborador) e `Fechado`; o mapeamento tag → categoria, a responsabilidade por status e os status finais são **globais** (não por sprint), e o cache de consulta é **um documento por sprint** (ver [Armazenamento](#armazenamento-dados-ou-firestore)) — consultar um sprint nunca sobrescreve o cache de outro.
...
- **Capacidade**: `diasUteis` (seg–sex no período, inclusive, **menos** `DiasNaoUteis`) → `tempoTotal = HorasPorDia × diasUteis` → `margem = floor(MargemPercentual/100 × tempoTotal)` → `TD = floor(tempoTotal − margem)` (tempo por colaborador) → `TD_i = max(0, TD − HorasDeduzidas[i])` → `CT = Σ TD_i` (capacidade total). `MargemPercentual` e `HorasPorDia` são obrigatórias no cadastro (400 se ausentes; margem 0 ≤ valor < 100); `DiasNaoUteis` (feriado etc.) fica entre 0 e os dias úteis do período. Ex.: `HorasPorDia=7`, `MargemPercentual=30`, `2026-09-01`→`2026-09-24` → `diasUteis=18`, `tempoTotal=126`, `margem=37`, `TD=89`.
- **Horas deduzidas** (`PUT /api/sprints/{chave}/deducoes`): horas inteiras a deduzir da capacidade de cada colaborador (férias, folga, atestado), por chave do usuário do Toggl; o cabeçalho traz `DiasNaoUteis` e cada `LinhaColaboradorSprint` traz `Chave`, `Td` já deduzido e `HorasDeduzidas`. Trava com o sprint fechado (409).
- **Tempo por tag** (`TempoPorTag`, gráfico do Acompanhamento): segundos por `Tags[0]` (ou "(sem tag)") somados de todos os colaboradores selecionados — inclui as tags detalhadas, agregadas pela tag —, do maior para o menor; calculado no mesmo `Montar`, sem chamada extra.
...
- **Busca e filtros** da tela são **client-side**, sobre a lista já carregada — sem nova chamada à API (a tela não ordena colunas).
...
`POST /api/planejamento/consultas` e `GET /api/planejamento` (`quadro=dev|analise`) alimentam a tela de Planejamento: os cartões do **sprint ativo do quadro Scrum** (de DEV ou de Análise) do Jira, por coluna do quadro, com a contagem de cartões por colaborador. Só Jira — não consulta o Toggl e **não depende do sprint do app** (nenhum parâmetro do sprint entra na composição); o dado é **global**, um cache por quadro, compartilhado pelas três entradas da tela.

- **Escopo**: `GET /rest/agile/1.0/board/{id}/sprint?state=active` → cartões via `GET /rest/agile/1.0/board/{id}/issue?jql=sprint in (ids)` (páginas de `maxResults=200` até `startAt >= total`; 429 com 1 retry respeitando `Retry-After`). Nunca busca o quadro inteiro (milhares de cartões) — exceto quadro **Kanban** (sem sprint; ex.: Análise), que traz tudo o que está no quadro (`/board/{id}/issue` sem JQL; o tipo vem de `GET /board/{id}` e o cache guarda `Kanban = true`, válido mesmo sem `SprintJira`). O Jira **não tem sprint equivalente ao do app** (o time agrupa "sprints" por épicos com o período no título), então o Planejamento **não correlaciona** os dois: mostra o sprint ativo do quadro e o nome dele.
...
- **Cache** `JiraPlanejamentoGlobalDev`/`JiraPlanejamentoGlobalAnalise` (um por quadro; dado **cru**: `QuadroId`, `QuadroNome`, `SprintJira`, `Colunas`, `Cartoes`, `AtualizadoEm` — coluna, contagens e siglas são calculadas em runtime). `POST .../consultas` reaproveita o cache, exceto com `forcar = true` ou sem cache válido (ausente, `QuadroId` diferente do configurado ou `SprintJira` nulo); o `GET` também descarta (409) cache de outro quadro. O estado Fechado de sprint **não se aplica**. Falha do Jira devolve 502 e mantém o cache anterior. O `GET` nunca dispara consulta.
- **Configuração** (`GET/PUT /api/jira/configuracao`): o quadro de cada tipo (`QuadroDevId`/`QuadroDevNome`, `QuadroAnaliseId`/`QuadroAnaliseNome`, em `JiraConexao`, escolhidos entre `GET /api/jira/quadros`) e o campo customizado "Analisado por" (`[Campo:AnalisadoPor]` em `JiraCampos`, user picker como "Revisado por") são obrigatórios para consultar aquele quadro — sem eles o `POST` devolve 400. "Time" (`[Campo:Time]`) é opcional e não bloqueia a consulta.

### Jira

A configuração vive em `JiraConexao` (URL, e-mail, `ApiToken` criptografado do mesmo jeito que o do Toggl e os quadros de DEV e de Análise do Planejamento) e `JiraCampos` (campos customizados: as três estimativas, "Revisado por", "Analisado por", "Time" e "Previsão de liberação"), mascarada (`ServicoUsuariosToggl.MascararToken`) em toda resposta. `ClienteApiJira` autentica com Basic `email:apiToken` contra `https://<dominio>/rest/api/3/`, normalizando a URL (com ou sem `https://`). `POST /api/jira/testar-conexao` sempre exige as três credenciais no corpo (valida exatamente o que está na tela); `POST /api/jira/campos` usa as do corpo ou, se vazio, as salvas, e filtra a `custom: true`.
...
**Cores**: a busca de issues não expõe cor de prioridade/status (só `statusCategory`), então a cor de cada badge vem de um mapeamento por nome exato (`GET/PUT /api/jira/cores`, guardado em `JiraStatus`/`JiraPrioridades`/`JiraColunasDev`/`JiraColunasAnalise`/`JiraTimes`/`JiraEpicos`), alimentado pela listagem real de nomes (as de colunas são as de cada quadro configurado, `GET /api/jira/colunas?quadro=`). Sem cor configurada, **Prioridade** cai numa paleta por severidade (Muito alta → Muito baixa) e **Status**/**Coluna** no cinza dos badges "Nenhuma".
...
- **Migração de formato** (`MigracaoDados`, no núcleo): roda na subida da API e logo após a restauração; lê o formato antigo, grava o novo e **apaga o antigo** (nada de órfão no Firestore). É idempotente e falha só vai para o log. Ao mudar o formato de um documento, acrescente o passo lá — os carregadores não têm lógica de compatibilidade.
...
Os demais documentos (configuração, cores, mapeamento, `Sprints`, `*Parametros`, `JiraSprintData_*`, `JiraPlanejamentoGlobal*` e os caches de listagens) não têm token.
