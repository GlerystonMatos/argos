...
Dois projetos **C# / .NET 10** na mesma solução (`Argos.slnx`):
...
| [`Argos.Nucleo/`](#núcleo-compartilhado-argosnucleo) | Biblioteca de classes | Modelos, acesso a INI e regras de negócio comuns |

A Web API consulta o Toggl Track, faz cache do retorno cru em arquivos `.ini` dentro de uma pasta `dados/` (ao lado do executável) e agrupa/busca sobre esse dado em runtime — toda a regra de negócio vive em `Argos.Nucleo`, referenciado pela API.
...
Compila os dois projetos (`Argos.Api`, `Argos.Nucleo`) de uma vez. `0 warnings` é o padrão esperado.
...
Biblioteca de classes referenciada pelo `Argos.Api`. Contém **tudo que não depende de HTTP** — toda a regra de negócio e o acesso a INI, sem nenhum `PackageReference`:
...
| `GET` | `/api/jira/colunas?forcarAtualizacao=` | Nomes reais das colunas do quadro de DEV configurado (`GET /rest/agile/1.0/board/{id}/configuration`), cache `JiraColunasCache.ini`; 400 sem configuração do Jira ou sem quadro selecionado |
| `GET` | `/api/jira/epicos` | Épicos (`{ chave, resumo }`) encontrados nos caches `JiraPlanejamentoData_*.ini` dos sprints **abertos**, sem chamar o Jira; sem repetição por chave. A cor salva de um épico que só está em sprint fechado continua em `JiraEpicos.ini` (só não é listada) e volta a aparecer se o sprint for reaberto |
| `GET` | `/api/jira/times` | Times (campo Time) encontrados nos caches `JiraPlanejamentoData_*.ini` de todos os sprints, sem chamar o Jira — o campo não tem listagem confiável na API |
...
| `JiraColunas.ini` | `[Coluna:<nome>]` — `Cor` (badge de Coluna do Planejamento) |
| `JiraTimes.ini` | `[Time:<nome>]` — `Cor` (badge de Time do Planejamento) |
| `JiraEpicos.ini` | `[Epico:<chave>]` — `Cor` (badge de Épico do Planejamento; pela chave, que não muda quando o título do épico muda) |
| `JiraQuadro.ini` | `[Geral]` — `ColunasOcultas` (JSON, lista de nomes de coluna que não aparecem no Planejamento) |
| `JiraTogglMapeamento.ini` | `[Usuario:<displayName do Jira>]` — `ChaveToggl`, `Sigla`, `Cor` |
| `Sprints.ini` | `[Sprint:<chave>]` — `Nome`, `HorasPorDia`, `MargemPercentual` (decimais, `InvariantCulture`; `MargemPercentual` sem valor salvo lê como `30`), `DataInicio`, `DataFim` (`yyyy-MM-dd`), `Fechado` (padrão `False`) |
...
| `TogglTagsCache.ini`, `JiraStatusCache.ini`, `JiraPrioridadesCache.ini`, `JiraColunasCache.ini`, `JiraUsuariosCache.ini`, `JiraQuadrosCache.ini` | caches de listagens reais (tags do Toggl; status, prioridades, colunas do quadro de DEV, usuários e quadros Scrum do Jira), renovados com `forcarAtualizacao=true` |
...
Os demais arquivos (`TogglConfiguracao.ini`, `TogglTags.ini`, `JiraCampos.ini`, `JiraStatus.ini`, `JiraPrioridades.ini`, `JiraColunas.ini`, `JiraTimes.ini`, `JiraEpicos.ini`, `JiraQuadro.ini`, `JiraTogglMapeamento.ini`, `Sprints.ini`, `*Parametros.ini`, `JiraSprintData_*.ini`, `JiraPlanejamentoData_*.ini` e os caches de listagens) não têm token.
...
As três env vars da API — `AUTH__USUARIO`, `AUTH__SENHA` (autenticação Basic opcional; ambas vazias = sem autenticação) e `CHAVE_CRIPTOGRAFIA` (vazia = chave padrão embutida) — vão no bloco `environmentVariables` de `Argos.Api/Properties/launchSettings.json`, já presente com valores vazios como template.
