...
| `GET/PUT` | `/api/jira/cores` | Cor (hex) por nome de status, de prioridade e de coluna: `{ coresStatus, coresPrioridade, coresColuna }` |
...
Os demais arquivos (`TogglConfiguracao.ini`, `TogglTags.ini`, `JiraCampos.ini`, `JiraStatus.ini`, `JiraPrioridades.ini`, `JiraColunas.ini`, `JiraQuadro.ini`, `JiraTogglMapeamento.ini`, `Sprints.ini`, `*Parametros.ini`, `JiraSprintData_*.ini`, `JiraPlanejamentoData_*.ini` e os caches de listagens) não têm token.
