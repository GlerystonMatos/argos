...
## Destruir o ambiente antigo (`toggl-report` → `argos`)

O ambiente provisionado antes do rename usa os nomes antigos — projetos
`toggl-report-app`/`toggl-report-finops`, serviços `toggl-report-back`/
`toggl-report-front`, repositório do Artifact Registry `toggl-report`,
trigger apontando para `toggl-report-infra/cloudbuild.yaml`. Ele **não é
migrado**: é destruído por completo e recriado do zero com a
[Ordem de execução](#ordem-de-execução) abaixo. Rode tudo aqui **antes** de
qualquer `terraform apply` com os nomes novos — um `apply` sobre o state
antigo tentaria substituir os recursos no meio do caminho.

**Não edite os `terraform.tfvars` nem apague os `terraform.tfstate` antes do
fim desta seção**: o state é o que diz ao Terraform o que existe; o `destroy`
age sobre os IDs gravados nele (não sobre os nomes atuais do código), mas o
provider usa o `*_project_id` do `tfvars`, que precisa continuar apontando
para os projetos antigos.

### D1. Conferir o que será destruído

```bash
cd argos-infra/terraform/app
...
terraform plan -destroy
```
O `plan -destroy` do `app` deve listar os 2 serviços Cloud Run (+ IAM
público), o repositório do Artifact Registry (com as imagens dentro), a
conexão/repositório/trigger do Cloud Build, a SA `cloud-build-deployer` com
seus bindings e as APIs; o do `finops`, a Cloud Function, o tópico Pub/Sub,
o bucket (+ zip) do código da function, a SA `billing-killswitch` e o
binding dela **na conta de faturamento** (este último não some apagando o
projeto — por isso o `destroy` do `finops` é obrigatório, não opcional).

### D2. Destruir o `app` (primeiro)

```bash
cd argos-infra/terraform/app
terraform destroy
```
**Destrutivo.** Vem primeiro para o killswitch continuar protegendo o
billing enquanto a aplicação ainda existe. Remove serviços, imagens, trigger
e a conexão com o GitHub.

### D3. Apagar o Budget (manual — fora do Terraform)
...
```
O Budget foi criado à mão (passo 3) e vive na conta de faturamento, não no
projeto; sem apagar, ele fica órfão apontando para um tópico que deixa de
existir no passo seguinte.

### D4. Destruir o `finops`

```bash
cd argos-infra/terraform/finops
terraform destroy
```
**Destrutivo.** Remove a function, o tópico, o bucket e — principalmente —
o `roles/billing.admin` da SA na conta de faturamento.

### D5. Apagar os projetos GCP

```bash
gcloud projects delete toggl-report-app
gcloud projects delete toggl-report-finops
```
**Destrutivo.** Leva junto o que o Terraform nunca gerenciou: secret do
token OAuth do GitHub no Secret Manager, buckets automáticos do Cloud
Build/Cloud Functions, SA padrão do Compute. O projeto fica 30 dias em
"pendente de exclusão" (recuperável com `gcloud projects undelete`) e o ID
**nunca mais pode ser reutilizado** — por isso o ambiente novo usa IDs novos.

### D6. Limpar o que ficou fora do GCP

- **GitHub**: renomear o repositório para `argos` (`github_repo` padrão em
  `terraform/app/variables.tf`) e, em *Settings → GitHub Apps* da conta,
  revisar o acesso do app **Google Cloud Build** (o passo 4 do ambiente novo
  refaz a autorização).
- **State local**: apagar `terraform.tfstate` e `terraform.tfstate.backup`
  das duas árvores — descrevem um ambiente que não existe mais.
- **`terraform.tfvars`**: trocar `finops_project_id`/`app_project_id` pelos
  IDs novos e remover qualquer sobrescrita comentada com nomes antigos.

Confirmação de que não sobrou nada: `gcloud projects list` não mostra os
dois projetos antigos (ou mostra com `DELETE_REQUESTED`) e
`gcloud billing budgets list` não traz o budget antigo. A partir daqui, siga
a [Ordem de execução](#ordem-de-execução) do passo 1.
...
```powershell
cd argos-infra\terraform\<finops|app>
...
(`argos-front`/`argos-back`, ainda com a imagem placeholder do
...
```bash
gcloud run revisions list --service=argos-back --region=us-central1
```

```bash
gcloud run revisions list --service=argos-front --region=us-central1
```
Lista as revisões existentes de cada serviço (cada deploy bem-sucedido cria
uma nova, mesmo sem tráfego apontado pra ela).

```bash
gcloud run services update-traffic argos-back --region=us-central1 \
  --to-revisions=REVISION_ANTIGA=100
```

```bash
gcloud run services update-traffic argos-front --region=us-central1 \
  --to-revisions=REVISION_ANTIGA=100
```
...
> mesmo valor —
> `gcloud run services update argos-back --region=us-central1 --project=SEU_PROJETO_APP_ID --update-env-vars=CHAVE_CRIPTOGRAFIA=SUA_CHAVE --remove-env-vars=TOGGL_CHAVE_CRIPTOGRAFIA`.
...
gcloud artifacts docker images delete \
  us-central1-docker.pkg.dev/SEU_PROJETO_APP_ID/argos/backend:SHA_ANTIGO --project=SEU_PROJETO_APP_ID
```
...
O ambiente antigo (`toggl-report-*`) ainda está no ar. Na ordem:

0. [Destruir o ambiente antigo](#destruir-o-ambiente-antigo-toggl-report--argos)
   por completo (D1–D6).
